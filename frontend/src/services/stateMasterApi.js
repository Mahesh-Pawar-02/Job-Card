...
const STATE_RESOURCE = `${API_BASE_URL}/state-master`;
...
export async function fetchAllStates() {
  const res = await fetch(STATE_RESOURCE, { method: 'GET' });
  return handleResponse(res);
}

export async function createState(state) {
  const res = await fetch(STATE_RESOURCE, {
    method: 'POST',
...
export async function updateState(id, state) {
  const res = await fetch(`${STATE_RESOURCE}/${id}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(state),
  });
...
export async function deleteState(id) {
  const res = await fetch(`${STATE_RESOURCE}/${id}`, { method: 'DELETE' });
  return handleResponse(res);
}
